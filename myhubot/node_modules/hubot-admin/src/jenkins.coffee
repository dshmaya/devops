...
module.exports = (robot) ->
  ...
  addCommittersToChannel = (channelId, items) ->
    ...
    for item in items
      ...
      for member in slackMembers
        if member.profile.email == slackMail
          utils.invite_user(robot,channelId,member.id)
          found = true
      if !found
  ...
  handleChannelForBuild = (job) ->
    ...
    if jobResult == "SUCCESS"
      ...
      robot.logger.info "need to remove room if exists"
      if robot.brain.get(jobName+'channel_id')
        utils.archive_channel_id(robot,robot.brain.get(jobName+'channel_id'))
        robot.brain.set jobName+'channel_id', undefined
    ...
    if jobResult == "FAILURE" || jobResult == "UNSTABLE"
      ...
      utils.add_channel(robot,'#' + name + robot.brain.get(jobName+'first_non_green_build'))
        .then (channel) ->
          ...
          robot.brain.set jobName+'channel_id', channel.id
          addCommittersToChannel(channel.id,job.changeSet.items)
