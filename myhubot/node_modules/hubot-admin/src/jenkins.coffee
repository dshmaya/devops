...
module.exports = (robot) ->
  ...
  try
    ...
    robot.logger.info 'Unable to read file'
  ...
  robot.hear /not me/i, (msg) ->
    msg.reply "feel free to leave this room by typing '/remove "+ msg.message.user.name + "'"

  robot.hear /on it/i, (msg) ->
    msg.reply "Thank you for taking responsibility. Which issue are you handling ? "
  ...
  getSlackMail = (jenkinsName) ->
    if jenkinsName.indexOf('.') > 0
      return jenkinsName+ '@hpe.com'
    else
      str = jenkinsName.split(/[\s,]+/)
      if(!str[1])
        return (str[0]+'@hpe.com').toLowerCase()
      return (str[1]+'.'+str[0]+'@hpe.com').toLowerCase()
  ...
  addCommittersToChannel = (channel, items) ->
    ...
    for item in items
      found = false
      slackMail = getSlackMail(item.author.fullName)
  ...
  handleChannelForBuild = (job) ->
    ...
    if jobResult == "SUCCESS"
      ...
      channel_name = robot.brain.get(jobName+'channel_name')
      if channel_name
        robot.adapter.customMessage getMsg2(channel_name,job) #send passed msg to the previous failed channel
        robot.send  {room: channel_name} , "build passed. you can archive this channel by typing: '/archive #"+ channel_name+"'"
      ...
      utils.add_channel(robot,'#' + name + "#" + robot.brain.get(jobName+'first_non_green_build'))
        .then (channel) ->
          ...
          utils.invite_user(robot,channel.id, "U19468UFN")
          utils.invite_user(robot,channel.id, "U1BRNPAEP")
          setTimeout ( ->
